...
bundled from `@fontsource` and imported in `main.tsx` rather than fetched from a font service. They're
served from Pages with the rest of the app, so a page load makes no third-party requests and doesn't
depend on a font service being reachable. They come from npm, not committed files like the card fonts,
because nothing needs their exact bytes: the card fonts must be the files the PDF embeds, so text that
fits in the preview fits in print. They're declared with `font-display: swap`,
