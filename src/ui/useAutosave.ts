 ...
 * `onSaved` after each save of an edit, but not after the save on mount, which only stores what opened.
 */
export function useAutosave(
  ...
  useEffect(() => {
    ...
    setFailed(!saved);
    if (saved && list !== opened.current) onSaved();
    // `onSaved` is left out: a new callback on a re-render isn't a change to save.
