...
export const vehicleProfileSchema = z
  ...
  .refine(unfittedMountsEmpty, {
    message: 'A Hull Option the Vehicle does not take holds nothing',
  });
...
/** The mounts of the Hull Options the Vehicle takes, in `vehicleMounts` order. */
export function fittedMounts({ turret, staticMount }: HullOptionFlags): VehicleMount[] {
  return vehicleMounts.filter((mount) => (mount === 'turret' ? turret : staticMount));
}

function unfittedMountsEmpty(
  profile: HullOptionFlags & { mounts: Record<VehicleMount, string | null> },
): boolean {
  const fitted = fittedMounts(profile);
  return vehicleMounts.every((mount) => fitted.includes(mount) || profile.mounts[mount] === null);
}
